...
data test1; 
    ...
    length ADDVAR $3 VAR $600;
    input ADDVAR $ VAR $;
    datalines;
FIR| A quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs.
SEC|A quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs. The five boxing wizards jump quickly. How vexingly quick daft zebras jump! Two driven jocks help fax my big quiz. The jay, pig, fox, zebra and my wolves quack!
...
data test2; 
    ...
    datalines;
SEC|A quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs, The five boxing wizards jump quickly. How vexingly quick daft zebras jump! Two driven jocks help fax my big quiz. The jay, pig, fox, zebra and my wolves quack!
THI|A quick brown fox jumps over the lazy dog Pack my box with five dozen liquor jugs, The five boxing wizards jump quickly. How vexingly quick daft zebras jump! Two driven jocks help fax my big quiz. The jay, pig, fox, zebra and my wolves quack!
FIR| A quick brown fox jumps over the lazy dog. Pack my box with five dozen liquor jugs.
 |A quick brown fox jumps over the lazy dog.
;
run;

* Generates dataset where pfx is derived from input variable ;
%splitvar(inds=test1,   var=VAR,    outds=split1);
* Generates dataset where pfx is set to NEW;
%splitvar(inds=test1,   var=VAR,    outds=split2,   pfx=NEW);
* Generates dataset where pfx is set to NEW and delimiter changed to dot;
%splitvar(inds=test1,   var=VAR,    outds=split3,   pfx=NEW,    dlm='.');
* Generate dataset where pfx is set to NEW, delimiter changed to dot and length to 100;
%splitvar(inds=test1,   var=VAR,    outds=split7,   pfx=NEW,    dlm='.',    len=100)

* Generates dataset where pfx is derived from input variable;
%splitvar(inds=test2,   var=VAR,    outds=split4);
* Generates dataset where pfx is set to NEW;
%splitvar(inds=test2,   var=VAR,    outds=split5,   pfx=NEW);
* Generates dataset where pfx is set to NEW and delimiter changed to dot;
%splitvar(inds=test2,   var=VAR,    outds=split6,pfx=NEW,dlm='.');

* Generate dataset where pfx is set to NEW, delimiter changed to dot and length to 100;
%splitvar(inds=test2,   var=VAR,    outds=split8,   pfx=NEW,    dlm='.',    len=100)
%splitvar(inds=test2,   var=VAR,    outds=split9,   pfx=NEW,    drop=N,     len=100,    debug=N)

* Test: Multiple runs required;
%splitvar(inds=test2,   var=VAR,    outds=split10,  pfx=NEW,    dlm=',',    len=25)
%splitvar(inds=split10, var=NEW9,  outds=split11,  pfx=NEW,    dlm=',',    len=25,     sfxst=9)
